...
import { makeSchedule, scheduleProjects, calculateScheduleMetrics } from "../lib";
  ...
  const tasks = useMemo(() => 
    optimizedSchedule.projects.flatMap(project => 
      project.allocations.map(allocation => ({
        ...
        duration: Math.ceil((allocation.endDate - allocation.startDate) / (24 * 60 * 60 * 1000)),
        progress: 0,
        allocation: allocation.percentage,
        assignedEngineers: [engineers.find(e => e.id === allocation.engineerId)?.name || 'Unknown']
      }))
    ),
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="mb-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            {Object.entries(metrics.engineerUtilization).map(([engId, utilization]) => (
                ...
                {metrics.conflicts.map((conflict, idx) => (
                  <li key={idx}>
                    {conflict.type === 'deadline_missed' && 
                      `${projects.find(p => p.id === conflict.projectId)?.name} misses deadline by ${
                        Math.ceil((conflict.actual - conflict.expected) / (24 * 60 * 60 * 1000))
                      } days`
                    }
                  </li>
                ))}
